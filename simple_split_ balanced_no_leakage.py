...
def generate_split_no_leakage(
    data_dir, split=[0.7, 0.1, 0.2], age_groupe=[35, 65], plot=True, interpolate=True
):
    # load data
    data = pd.read_parquet(data_dir)
    # remove unnanotated entries
    data = data[data.annotated == 0]
    data["RestingECG_PatientDemographics_PatientID"] = [
        str(i).zfill(7) for i in data["RestingECG_PatientDemographics_PatientID"].tolist()
    ]
    ...
    for index, patient_id in tqdm(
        zip(data.index.tolist(), data["RestingECG_PatientDemographics_PatientID"].tolist()),
        total=len(data["RestingECG_PatientDemographics_PatientID"].tolist()),
    ):
    ...
    if plot == True:
        ...
        sns.histplot(
            data["RestingECG_PatientDemographics_PatientAge"].tolist(), binwidth=10, ax=axs[1]
        )
        ...
        sns.histplot(
            data_unique["RestingECG_PatientDemographics_PatientAge"].tolist(),
            binwidth=10,
            ax=axs[1],
        )
    ...
    X_train, X_test, y_train, y_test = multilabel_train_test_split(
        y_target, to_balance, stratify=to_balance, test_size=1 - split[0]
    )
    X_val, X_test, y_val, y_test = multilabel_train_test_split(
        X_test, y_test, stratify=y_test, test_size=split[2] / (split[1] + split[2])
    )
    ...
    train_set, val_set, test_set = regenerate_the_dataset(
        X_train, X_val, X_test, dict_observations, data
    )
