    ...
    with tqdm(total=df.shape[0]) as pbar:
        for pos in range(df.shape[0]):
            if len(df.iloc[pos]["Lead_Wavform_1_ID_I"]) == 5000:
                arr_ = np.array(
                    [
                        df.iloc[pos]["Lead_Wavform_1_ID_I"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_II"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_III"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_V1"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_V2"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_V3"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_V4"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_V5"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_V6"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_aVL"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_aVR"][1::2],
                        df.iloc[pos]["Lead_Wavform_1_ID_aVF"][1::2],
                    ]
                )
            else:
                arr_ = np.array(
                    [
                        df.iloc[pos]["Lead_Wavform_1_ID_I"],
                        df.iloc[pos]["Lead_Wavform_1_ID_II"],
                        df.iloc[pos]["Lead_Wavform_1_ID_III"],
                        df.iloc[pos]["Lead_Wavform_1_ID_V1"],
                        df.iloc[pos]["Lead_Wavform_1_ID_V2"],
                        df.iloc[pos]["Lead_Wavform_1_ID_V3"],
                        df.iloc[pos]["Lead_Wavform_1_ID_V4"],
                        df.iloc[pos]["Lead_Wavform_1_ID_V5"],
                        df.iloc[pos]["Lead_Wavform_1_ID_V6"],
                        df.iloc[pos]["Lead_Wavform_1_ID_aVL"],
                        df.iloc[pos]["Lead_Wavform_1_ID_aVR"],
                        df.iloc[pos]["Lead_Wavform_1_ID_aVF"],
                    ]
                )
            arr_df.append(arr_)
            pbar.update(1)
...
def add_labels_FA_RS(data_set):
    ...
    for pos, i in enumerate(data_set["Diag"]):
        if (
            "Fibrillation auriculaire" in i
            and "**" not in i
            and "ECG normal" not in i
            and "*Analyse impossible; aucun QRS décelable* " not in i
            and "Rythme sinusal normal" not in i
        ):
            list_labels_string.append("FA")
            list_labels_num.append(0)

        elif (
            "Rythme sinusal normal" in i
            and "**" not in i
            and "*Analyse impossible; aucun QRS décelable* " not in i
            and "Fibrillation auriculaire" not in i
        ):
...
def balanced_w_age_sex(df, split=[0.6, 0.2, 0.2], plot_distr=True, num_age_bin=10):
    ...
    df_bined["combined"] = (
        df_bined["bined_age"].astype(str)
        + "_"
        + df_bined[age_var].astype(str)
        + "_"
        + df_bined["FA_RS_str"].astype(str)
    )
    ...
    X_train, X_val_test, y_train, y_val_test = train_test_split(
        df_bined, y, test_size=0.4, stratify=y
    )
