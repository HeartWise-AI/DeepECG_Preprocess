...
data_["RestingECG_PatientDemographics_PatientID"] = [
    str(i).zfill(7) for i in data_["RestingECG_PatientDemographics_PatientID"].tolist()
]
...
def regenerate_the_dataset(
    train_data, val_data, test_data, dict_observations, original_dataframe
):
...
def adjust_private_variables_to_balance(df, sex_groups=[35, 65]):
    df["SEX_BIN"] = [
        0 if i == "FEMALE" else 1 for i in df["RestingECG_PatientDemographics_Gender"].tolist()
    ]
    df["AGE_BIN"] = [
        0 if i < sex_groups[0] else 2 if i > sex_groups[1] else 1
        for i in df["RestingECG_PatientDemographics_PatientAge"].tolist()
    ]
...
def transform_(dataset, name, split=0):
    if (
        not os.path.exists(f"/media/data1/anolin/split_smaller_for_ram/{name}")
        and name == "final"
    ):
        os.mkdir(f"/media/data1/anolin/split_smaller_for_ram/{name}")

    if (
        not os.path.exists(f"/media/data1/anolin/split_smaller_for_ram/split_{split}")
        and name != "final"
    ):
        os.mkdir(f"/media/data1/anolin/split_smaller_for_ram/{name}")
    ...
    for path, labels in enumerate(
        tqdm(
            zip(dataset["npy_path"], dataset.index),
            total=len(dataset),
            desc="Generating X,Y data for final set",
        )
    ):
    ...
    if name == "final":
        np.save(
            f"/media/data1/anolin/split_smaller_for_ram/{name}/{name}_Y.npy", labels.to_numpy()
        )

        out_ = np.array(temp_data).astype(np.float32)
        print(f"final shape {out_.shape}")
        np.save(f"/media/data1/anolin/split_smaller_for_ram/{name}/{name}_X.npy", out_)

    else:
        np.save(
            f"/media/data1/anolin/split_smaller_for_ram/split_{split}/{name}_Y.npy",
            labels.to_numpy(),
        )

        out_ = np.array(temp_data).astype(np.float32)
        print(f"final shape {out_.shape}")
        np.save(f"/media/data1/anolin/split_smaller_for_ram/split_{split}/{name}_X.npy", out_)
...
def generate_balanced_split(
    df,
    seed_list=[420, 1997, 2023],
    sex_groups=[35, 65],
    num_splits=3,
    notebook=False,
    outdir="/volume/core_model",
    **kwargs,
):
    ...
    for index, patient_id in tqdm(
        zip(df.index.tolist(), df["RestingECG_PatientDemographics_PatientID"].tolist()),
        desc="Generating patient:ECG dict",
        total=len(df["RestingECG_PatientDemographics_PatientID"].tolist()),
    ):
    ...
    start_col = 280
    end_col = 357
    to_balance = data_unique[
        data_unique.columns.tolist()[start_col : end_col + 1] + ["SEX_BIN", "AGE_BIN"]
    ]

    X_data, X_final, y_data, y_final = multilabel_train_test_split(
        data_unique, to_balance, stratify=to_balance, test_size=0.30, random_state=int(8.30**2)
    )
    ...
    for pos in tqdm(range(num_splits), desc="Generating splits"):
        try:
            os.mkdir(f"/media/data1/anolin/split_smaller_for_ram/split_{pos}")

        except:
            print(
                colored(f"/media/data1/anolin/split_smaller_for_ram/split_{pos}", "red"),
                "already exists",
            )

        seed_everything(seed_list[pos])

        X_train, X_val_test, y_train, y_val_test = multilabel_train_test_split(
            X_data, to_balance, stratify=to_balance, test_size=0.15, random_state=seed_list[pos]
        )
        X_val, X_test, y_val, y_test = multilabel_train_test_split(
            ...
            stratify=y_val_test,
            test_size=0.66,
            random_state=seed_list[pos],
        )

        X_train, X_val, X_test = regenerate_the_dataset(
            X_train, X_val, X_test, dict_observations_2, df
        )

        # save the files
        X_train.to_csv(
            os.path.join(f"/media/data1/anolin/split_smaller_for_ram/split_{pos}", "X_train.csv")
        )
        X_val.to_csv(
            os.path.join(f"/media/data1/anolin/split_smaller_for_ram/split_{pos}", "X_val.csv")
        )
        X_test.to_csv(
            os.path.join(f"/media/data1/anolin/split_smaller_for_ram/split_{pos}", "X_test.csv")
        )
