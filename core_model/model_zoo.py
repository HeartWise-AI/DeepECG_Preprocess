...
def block_1_1(prime_service):
    ...
    for split in range(3):
        ...
        model.compile(optimizer=optimizer_, loss=loss_func, metrics=['categorical_accuracy',
                                                                    ...
                                                                    tf.keras.metrics.SensitivityAtSpecificity(0.5,name='sens')], run_eagerly=False)
        ...
        X_train = np.load("/media/data1/anolin/split_smaller_for_ram/split_{}/train_X.npy".format(split))
        Y_train = np.load("/media/data1/anolin/split_smaller_for_ram/split_{}/train_Y.npy".format(split))
        ...
        dict_results = dict()
        for pos,i in enumerate(['loss','categorical_accuracy','binary_accuracy','AUC','PR','F1_micro','F1_macro','Sensitivity','Specificity']):
        ...
        with open(os.path.join(prime_service['save_dir'],wandb.config.models,'split_{}'.format(split),'saved_outputs_detailed.pickle'), 'wb') as handle:
